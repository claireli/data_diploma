...
shinyServer(function(input, output, session) {
  ...
  output$UnivariateFit<-renderPrint({
    ...
    unistuff<-paste(input$Rate," ~ ", input$uni1)
    unifit<-lm(unistuff, data = render_grad())
    summary(unifit)
    }
  })
  output$BivariateFit<-renderPrint({
    ...
    bistuff<-paste(input$Rate," ~ ", input$bi1, " + ", input$bi2)
    bifit<-lm(bistuff, data = render_grad())
    summary(bifit)
    }
  })
  output$MultivariateFit<-renderPrint({
    ...
    multistuff<-paste(input$Rate," ~ ", input$multi1, " + ", input$multi2, " + ", input$multi3)
    multifit<-lm(multistuff, data = render_grad())
    summary(multifit)
    }
  })
